...
# Крым не является спецзоной по тарифу — там обычный тариф
SPECIAL_POLYGONS = [
    # ДНР
    [(48.07,37.45),(48.35,38.15),(48.55,38.85),(48.1,39.5),
     (47.8,39.6),(47.4,38.9),(47.1,38.2),(47.3,37.5),(47.6,37.1),(47.9,37.2),(48.07,37.45)],
    # ЛНР
    ...
    [(47.6,34.2),(47.9,35.1),(47.85,36.0),(47.6,36.8),
     (47.3,37.1),(47.0,36.5),(46.7,35.8),(46.6,34.8),(46.9,34.2),(47.3,33.9),(47.6,34.2)],
    # Херсонская
    [(47.0,32.5),(47.2,33.5),(46.9,34.2),(46.6,34.8),
     (46.4,34.4),(46.35,33.6),(46.4,32.8),(46.6,32.3),(47.0,32.5)],
]
...
def osrm_route_with_geometry(coords: list) -> dict:
    ...
    coords — список (lon, lat).
    Возвращает {"distance_km": float, "waypoints": [(lat,lon), ...]} или None.
    """
    try:
        ...
        # Геометрия — список [lon, lat]
        pts = route["geometry"]["coordinates"]
        waypoints = [(pt[1], pt[0]) for pt in pts]  # → (lat, lon)
...
def fallback_distance(fc, tc) -> tuple:
    """Fallback когда OSRM недоступен — haversine×1.4 + определение зоны по конечным точкам."""
...
def handler(event: dict, context) -> dict:
    """Рассчитать стоимость поездки. Спецрегионы считаются как РФ — один OSRM-запрос на весь маршрут, км спецзоны определяются по геометрии маршрута."""
