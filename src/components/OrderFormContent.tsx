  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className={`w-full px-4 ${compact ? "py-2 text-sm" : "py-3 text-sm"} bg-[#2a2a2a] rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#9aab2a]/60 transition ${showGeo ? "pr-10" : ""}`}
      />
      {showGeo && (
        <button
          type="button"
      ...
      <form onSubmit={p.handleSubmit} noValidate className={`flex flex-col justify-between flex-1 h-full min-w-0 ${c ? "gap-1.5" : "gap-3"}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <input
              ...
              className={`w-full px-4 ${c ? "py-2 text-sm" : "py-3 text-sm"} bg-[#2a2a2a] rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#9aab2a]/60 transition`}
            />
            ...
            {CAR_CLASSES.map((cls) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center ${c ? "gap-0.5 px-2.5 py-1.5" : "gap-0.5 px-3 py-2.5"} rounded-xl transition-all ${c ? "min-w-[52px]" : "min-w-[56px]"} ${
                    p.carClass === cls.id
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`${c ? "w-9 h-9" : "w-10 h-10"} flex items-center justify-center bg-[#2a2a2a] rounded-full shrink-0`}
            title="Способ оплаты"
          >
            <Icon name="Wallet" size={c ? 16 : 18} className="text-[#c8d44a]" />
          </button>
          <button
            type="submit"
            className={`flex-1 bg-[#9aab2a] hover:bg-[#b0c430] text-black font-bold ${c ? "text-sm py-2" : "text-sm py-3"} rounded-full transition-colors duration-200 shadow-lg flex flex-col items-center leading-tight`}
          >
            {p.priceLoading ? (
          ...
          <button
            ...
            title="Доп. услуги"
          >
            <Icon name="SlidersHorizontal" size={c ? 16 : 18} className="text-[#c8d44a]" />
          </button>
        </div>
